...
def echo_last_find(bot: TeleBot, keybord_options):
    ...
    def find_last_keyboard(message):
        if message.chat.type == "private":
            ...
            bot.send_message(
                message.chat.id, "Данная команда предназначена только для использования в личных сообщениях."
            )
    ...
    def keyboard_callback_handler(call):
        ...
        if command == 'last':
            ...
            if selected == 4:
                ...
                send_entries_from_all_seminars(bot, limiter, chat_id, seminars, entries, news, info_text, hashtags)
            else:
                hashtag = '\n' + hashtags[selected] + ' ' + hashtags[-1]
                info_text = info_text.format(seminars[selected])
                send_entries_from_one_seminar(bot, limiter, chat_id, selected, entries, news, info_text, hashtag)
            ...
            bot.send_message(chat_id, "Пожалуйста, отправьте поисковый запрос следующим сообщением.")
...
def echo_notify(bot: TeleBot, states_file):
    ...
    def handle_notify(message):
        ...
        if user_id in authorised_users:
            ...
            bot.reply_to(message, 'Для этого нужно быть модератором или администратором бота.')
        ...
        if user_id in user_states and "prompt" in user_states[user_id]:
            ...
            if selected == 4:
                if any(sublist for sublist in matching_indexes):
                    ...
                    send_entries_from_all_seminars(bot, limiter, chat_id, seminars, matching_indexes, news, info_text)
                else:
                    bot.send_message(chat_id, "Ничего не найдено.")
            else:
                matching_indexes = matching_indexes[selected]
                if matching_indexes:
                    info_text = "Найдено среди семинаров '{}':".format(seminars[selected])
                    send_entries_from_one_seminar(bot, limiter, chat_id, selected, matching_indexes, news, info_text)
                else:
                    bot.send_message(chat_id, "Ничего не найдено.")
