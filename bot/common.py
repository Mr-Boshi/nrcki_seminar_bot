...
def send_entries_from_one_seminar(
    bot, limiter, chat_id, seminar_index, indexes, news, info_text=None, hashtags=None
):
    if indexes:
        ...
        for index in indexes:
            while not limiter.ready():
                time.sleep(0.5)
            send_single_entry(
                bot, limiter, chat_id, seminar_index, index, news, hashtags
            )
...
def send_entries_from_all_seminars(
    bot, limiter, chat_id, seminars, indexes_list, news, info_text=None, hashtags=None
):
    for seminar_index, sublist in enumerate(indexes_list):
        ...
        send_entries_from_one_seminar(
            bot, limiter, chat_id, seminar_index, sublist, news, info_text, hashtag
        )
