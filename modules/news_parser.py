...
def parse_seminars_soup(soup):
    ...
    indexes_of_years = [
        index
        for index, string in enumerate(filtered_empty_paragraphs)
        if re.match(year_pattern, string)
    ]
    filtered_paragraphs = filtered_empty_paragraphs[indexes_of_years[0] :]

    indexes_of_dates = [
        index
        for index, string in enumerate(filtered_paragraphs)
...
def pagagraphs_md(paragraph):
    ...
    pattern_italic = re.compile(
        r"\b(" + "|".join(escaped_italic_words) + r")\b", re.IGNORECASE
    )
    pattern_bold = re.compile(
        r"\b(" + "|".join(escaped_bold_words) + r")\b", re.IGNORECASE
    )
